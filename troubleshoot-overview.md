...
* Make sure that your command-line tools are up-to-date.
    ...
    * When you use any of the CLI `get` commands, such as the  **`app get`**, **`job get`**, **`jobrun get`**, **`build get`**, or **`buildrun get `** commands, you can specify the `--o yaml` option to obtain more fine-grained details about your {{site.data.keyword.codeengineshort}} component. For more information about the `get` commands, see [CLI Reference](/docs/codeengine?topic=codeengine-cli).
