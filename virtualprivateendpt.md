...
2. Make sure that your VPC has at least one VSI (virtual server instance) and can connect to the VSI. You can use the VPC console, CLI, and API to  provision a VSI from the {{site.data.keyword.vpc_full}} page in the {{site.data.keyword.cloud_notm}} console. 

    1. Create an [SSH key](/docs/vpc?topic=vpc-ssh-keys) to access the VSI. 
    2. [Create a virtual server instance by using the console](/docs/vpc?topic=vpc-creating-virtual-servers).
