...
# NEW Accessing container registry
{: #add-registry-KR}
...
if your registry is public, you do not have to set up authorities in order to pull images. Note that pulling images from a public registry while you are getting started with Code Engine is acceptable, use a private registry when it comes to your enterprise workloads.

**What authorities do I need?**

When you deploy apps or run jobs from the console, Code Engine can automatically access images that are in your own account. If you want to access images from a shared account, other IBM Cloud accounts, or a private Docker acount, you must have the proper authorities.
...
| Push images | `Read` and `write` access | When you build source code, you must push the image to a registry. To push images, you need `write` access to {{site.data.keyword.registryfull_notm}}. You cannot push images to a different type of registry. |
...
Yes, you can create a service ID and assign authorities to it. Service IDs are also automatically created by the Code Engine UI when automatically creating access to your {{site.data.keyword.registryfull_notm}}. DO NOT delete this service ID as you will lose access to the images in the registry.

**Can I access images in a different registry?**

Yes! Here is how.

- Accessing images from public account
- Accessing images in your own account
...
If you are accessing {{site.data.keyword.codeengineshort}} from the CLI, you must first create an IAM API key and then [save the IAM API key as registry access](#add-registry-access-ce) in {{site.data.keyword.codeengineshort}}.
...
Now that you created your API key, you can use it directly in the `app create` or `job create` commands or you can [save the API key as registry access](#add-registry-access-ce).
...
If you are plan to deploy apps and run jobs from the shared account, {{site.data.keyword.codeengineshort}} can pull or push images for you when you deploy your application or create your job.

If you want to pull images from the shared account to your own account, then you must first [create an API key](#images-your-account-api-key) in the shared account, and then use that key to [create access](#add-registry-access-ce) in your account.
...
To access images in a private Docker Hub account, create registry access by proving your password or an access token. By using an access token, you can more easily grant and revoke access to your Docker Hub account without requiring a password change. For more information about access tokens and Docker Hub, see [Managing access tokens](https://docs.docker.com/docker-hub/access-tokens/){: external}.  
...
To set up access to an {{site.data.keyword.registryfull_notm}} in a different {{site.data.keyword.cloud_notm}} account, to pull images from a private Docker Hub account, or to pull or push images by using the {{site.data.keyword.codeengineshort}} CLI, you can use the [IBM API key]( or Docker Hub access token to create registry access through {{site.data.keyword.codeengineshort}} to store your authentication key or token for you.
