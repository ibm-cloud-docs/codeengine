...
| ----- | ------- | -------------- |
| 0.5.25 | 17 March 2021 | <ul><li>Added the `project update` command.</li><li>Added support for custom {{site.data.keyword.cloud_notm}} Operator service ID when service binding. </li><li>Updated the `subscription ping create` and `subscription ping update` commands to add the following options:  `--content-type`, `--time-zone`, `--data`, and `--data-base64`.</li><li>Updated the default value for the `--cpu` option on the `job create` and `jobrun submit` commands to `0.1`.</li></ul>|
