...
| ----- | ------- | -------------- |
| 0.5.17 | 15 February 2021 | <ul><li>Added support for delegated refresh tokens for improved security when obtaining the project configuration. This action occurs when you use the `project select` command, or when you use the `project create` command and do not specify the `--no-select` option.</li></ul>|
