...
| --------- | -------- | -------- |
| 0.3.1712 | 17 August 2020 | <ul><li>Improved trace output for Kubernetes API calls and errors.
</li><li>Fixed merge error with multiple `kubeconfig` files during target.</li><li>Added support for configmap and secret `--from-file` option to accept alias.</li><li>The `job run` and `jobdef` now supports private registries.</li><li>Added the `--concurrency-target` option to `app create` and `app update` commands.</li></ul> |
