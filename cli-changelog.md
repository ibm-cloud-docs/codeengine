...
| --------- | -------- | -------- |
| 0.3.1675 | 14 August 2020 | <ul><li>Deprecated `--arraysize`. Use `array-indices` instead.</li><li>Added `job rerun` command to rerun a job based on the configuration of a previous job run.</li><li>If you do not specify values for `cpu` and`memory` when running a job, the defaults from the jobdef are used.</li><li> The 'job create' command is now an alias for 'job run' command.</li></ul> |
