...
:   With {{site.data.keyword.codeengineshort}}, you can now go from source code to a running app or job with a **single** command. You don't even need to know about building container images or container management. Just let {{site.data.keyword.codeengineshort}} handle all these processes for you.
    * If you are familiar with push from source platforms, get started with the tutorial [Deploying Cloud Foundry applications in Code Engine: Getting started](/docs/codeengine?topic=codeengine-migrate-cf-ce-tutorial). 
