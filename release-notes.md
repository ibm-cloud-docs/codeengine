...
:   With {{site.data.keyword.codeengineshort}}, you can now go from source code to a running app or job with a **single** command. The **`app create`** command automatically runs the container build and deploys your app in a single step. You don't even need to know about building container images or working with registries. Just let {{site.data.keyword.codeengineshort}} handle all these processes for you.
    * If you are familiar with "push source code" platforms, such as Cloud Foundry, you can get started with the tutorial [Deploying Cloud Foundry applications in Code Engine: Getting started](/docs/codeengine?topic=codeengine-migrate-cf-ce-tutorial). 
